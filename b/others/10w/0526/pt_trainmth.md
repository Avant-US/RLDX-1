...
- 这说明：VLA/WAM 预训练里，**真机数据**和**代理数据**（人类视频、仿真、Web 视频等）不能直接按小时数相加，要先换算成统一的「有效数据量」再比较。        

| 符号 | 含义 |
|------|------|
| \($D_{\text{eff}}$\) | **有效数据规模**：用来横向对比不同 PT 配方的统一指标 |
| \($D_{\text{real}}$\) | **真机数据**：真实机器人交互轨迹（小时数、episode 数等） |
| \($D_{\text{proxy}}$\) | **代理数据**：能部分替代真机的数据，如 EgoDex 人类第一视角视频、仿真、Web 视频 |
| \(\lambda\) | **折算系数**：1 单位代理数据 ≈ \(\lambda\) 单位真机数据的「学习价值」 |

\($\lambda \in (0.1, 1.0)$\) 表示：代理数据通常**不如真机值钱**（\($\lambda < 1$\)），但也不是零（下限约 0.1）；仿真等高质量代理有时可接近真机（上限约 1.0）。

#### 括号里的 Ψ0 例子

**「Ψ0 EgoDex 800h 折算约 ≈ 0.5 × 真机」** 意思是：

- Ψ0 用了 **800 小时 EgoDex**（人类操作的第一视角视频）做 AR 预训练，再用约 **30 小时真机**做 Flow 后训。
- 在这套分析框架里，EgoDex 的折算系数约 **\(\lambda \approx 0.5\)**：  
  **1 小时 EgoDex ≈ 0.5 小时真机等效数据**。
- 因此 800h EgoDex 在有效规模上大致相当于 **400h 真机**；再加上 30h 真机本身，  
  \($D_{\text{eff}} \approx 30 + 0.5 \times 800 = 430$\) 小时（真机等效）。

文档第 6 章还给了更细的拆分（人类视频 \($\lambda_{\text{human}} \approx 0.3\text{–}0.5$\)，仿真更高、纯 Web 视频更低），并引用 Ψ0 消融：**用远少于纯真机基线的数据，仍明显超过 10× 数据基线**（约 +40pp）。

#### 一句话总结

这是在说：**比较 PT 数据规模时，不能只看「800h 人类视频 + 30h 真机」的原始数字，而要用 \(\lambda\) 把代理数据折成真机等效量 \(D_{\text{eff}}\)**；Ψ0 的 EgoDex 按约 0.5 折算，说明人类视频能显著「放大」有效数据，但通常仍不能完全等同真机。
...
- **RoboTwin 2.0** 的表现 STARRY > Fast-WAM(Galaxea, 开源) > GigaWorld-Policy(开源) 
