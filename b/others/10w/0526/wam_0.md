...
视频帧与动作命令交织为单一序列，自回归预训练 1.4T token。LIBERO long-sequence 98.5%，RoboTwin 2.0 双臂 90%+（baseline 90.2%）。在光照/噪声/布局扰动下鲁棒率 74.2%。但**相机视角和初始状态变化仍是软肋**——视频先验对几何配置变化无能为力。[4]

**Consistency-Consensus 子段（2026-05 NYCU）**：诊断研究 [60] 发现 LingBot-VA 作为 **inverse-dynamics WAM** 代表，其 action-state consistency \(c_t = \exp(-\alpha \cdot d_{\text{MSE-latent}}(o_{t+\Delta}, \hat{o}_{t+\Delta}))\) 与任务成功率高度相关（**Cohen's d = 0.99，AUC = 0.88**）。基于此提出 **value-free consensus 选择**：每步采 K 次 rollout，按 consistency 排序选最高，**无需 reward 模型 / 无需训练即让 LingBot-VA 在 RoboTwin 2.0 从 90.2% → 93.0%（+2.8 pp）**；同时揭示 **background collapse** 失败模式（低动态轨迹静态预测取虚假高 consistency，需用 Δz_t 诊断）。这是首次系统说明**当前 WAM 已具备「内部一致 ≈ 决策可靠」属性，可被 zero-training 利用**。[60]
...
**Vidu VAE + 5B Vidu DiT 基座 + 三流 MoT（text/video/action 独立 transformer）+ H-Bridge attention**——中间 50% 层做 V-A 全联合注意、两端 25% 层 decoupled，**同一模型支持 5 种推理模式**（VLA / WM / IDM / VGM / Joint）。**四层数据金字塔**（Internet → ego-centric → heterogeneous embodiment → specific embodiment）+ 两阶段 PT（Stage1 仅训视频分支、Stage2 训动作分支冻视频）。Action 10-D（pos + 6D rot + gripper）相对 EEF。**RoboTwin 2.0 clean 95.8% / randomized 96.1% 综合 #1**；**WorldArena 最强 EWMScore**；新本体仅 50-100 demos 适配。推理 stack：denoising 步数缩减 + V2A 异步（仅出 action 分支）+ chunk-level RTC + torch.compile + **FP8 量化（per-tensor float8_e4m3fn）+ DiT cache**，端到端 **≥ 50× 加速**。消融：**H-Bridge 中间联合层**是跨模态对齐关键；**LingBot-VA-style noisy-conditioning**（s_aug ∼ U[0.3, 0.7]）+ 多视角随机 drop p=0.1 提升鲁棒性。[61]

### 1.9 X-WAM（清华 + 小米）

**Wan2.2-TI2V-5B 视频 DiT 基座 + 轻量深度分支（复制 final DiT blocks）+ 多视角 RGB-D 联合预测 + Asynchronous Noise Sampling（ANS）**。两大创新：(1) **结构性轻量 3D 适配**——不沿序列拼深度 token（避二次 attention 代价）、不沿通道拼（避破坏预训练分布），改为「复制最后几层 DiT 作 dedicated depth branch」保留视觉先验；(2) **ANS**——训练时从 (t_O, t_a) 联合分布采样，与推理时「动作少步、视频多步」分布对齐，消除独立采样 train-test gap。Pretrained on **5,874 h / 1.49M episodes** robot data。**RoboCasa SOTA 79.2% / RoboTwin 2.0 90.7%**；**4D 重建 visual & geometric 指标均超过 Cosmos-Policy / UWM / Motus 等 2D unified WAM**。消融关键：**深度监督同时提升 3D 重建质量与策略成功率**——直接验证「spatial supervision 多目标受益」，与 WorldArena 报告的 perception-functionality gap 反向呼应：**只要 3D 监督 explicit 且与视频共享 backbone，画质与具身有用性可同涨**。[62]
...
**2026 H1 三个新观察**：
1. **MotuBrain [61] 把 RoboTwin 2.0 推到 95.8/96.1 接近天花板**——证明同一模型可同时是 VLA / WM / IDM / VGM / Joint 五种模式，且推理 50× 加速后仍可生产部署。
2. **X-WAM [62] 引入 RGB-D 4D 监督**——首个 unified WAM 同时在 RoboCasa SR 与 3D 重建几何指标上 SOTA，回应 WorldArena 的 perception-functionality gap。
3. **Consistency-Consensus [60] 把 WAM 的 "内部一致性" 变成零训练 +2.8 pp 工具**——预示未来 WAM 评估与策略选择有可能完全不需要 reward 模型。
...
### 12.3 ACoT-VLA（AgiBot，2026.01）

**Action Chain-of-Thought**：把 "thought" 重定义为「结构化 explicit action intents」，**首个把 CoT 引入动作空间** 而非语言/视觉空间。基座 π0.5（SigLIP + Gemma-2B 18 层 d=2048），新增两个 reasoner：
- **EAR (Explicit Action Reasoner)**：18 层轻量 Transformer，self-attn 捕时间依赖 + cross-attn 从 VLM 各层 KV cache 注入多模态先验，flow matching 输出 coarse 参考轨迹 a^ref（H_ref=15, shift=2）。
- **IAR (Implicit Action Reasoner)**：每 VLM 层学一个查询 Q_i（M=1, d'=128 降维），cross-attn 提取潜在动作先验。

**作为 vision/language CoT 的替代**：论文 §1 明确把 World-model-based policies（CoT-VLA / DreamVLA / WorldVLA 等）作为对比基线，指出 vision-level guidance 仍是 indirect representation；本文用动作空间 explicit + implicit reasoner 取代 WM 提供的视觉/语言 intermediate。**LIBERO 平均 +1.6 pp over π0.5**；**LIBERO-Plus 2026 H1 SOTA 87.5%**；**VLABench unseen-texture IS +12.6 pp / PS +7.2 pp**。消融：EAR 单独 +1.2 pp、IAR 单独 +1.2 pp。8×H100 单节点训练，单卡 RTX 4090 推理。[63]
...
9. **Unified 多模式 WAM 可同模型支持 VLA / WM / IDM / VGM / Joint 五种推理**（MotuBrain [61]）——三流 MoT + H-Bridge 是关键架构选择

10. **RGB-D 4D 监督 + 异步去噪同时提升 3D 重建与策略 SR**（X-WAM [62]）——首次反驳 WorldArena perception-functionality gap，**画质与具身有用性可同涨**

11. **WAM 内部 action-state consistency 可作零训练 test-time ranking 信号**（Consistency-Consensus [60]）——AUC 0.77–0.88，**RoboTwin 2.0 +2.8 pp 无新参数**

12. **动作空间 CoT > 视觉/语言 CoT**（ACoT-VLA [63]）——EAR + IAR 在 LIBERO-Long 显著领先 visual CoT（CoT-VLA / DreamVLA）
...
8. **Consistency 在 background collapse 情形会反向**（Consistency-Consensus [60]）——低动态轨迹的静态预测取虚假高 consistency，需 Δz_t 诊断后再用
...
- [60] Is the Future Compatible? Diagnosing Dynamic Consistency in World Action Models（LingBot-VA + Consistency-Consensus）. NYCU. arXiv:2605.07514, 2026.
- [61] MotuBrain: An Advanced World Action Model for Robot Control. Xiaomi. arXiv:2604.27792, 2026.
- [62] X-WAM: Unified 4D World Action Modeling from Video Priors with Asynchronous Denoising. Tsinghua + Xiaomi. arXiv:2604.26694, 2026. Project: https://sharinka0715.github.io/X-WAM/
- [63] ACoT-VLA: Action Chain-of-Thought for Vision-Language-Action Models. AgiBot + Beihang. arXiv:2601.11404, 2026. GitHub: https://github.com/AgibotTech/ACoT-VLA
