...
- **数据规模**：LIBERO 四套件 + ManiSkill2 + RoboTwin 2.0 + 真机 AgileX/Galaxea-R1-Lite 任务（每类 45–60 demo）
...
- **比 Pi0.5 好**：(40pp)真机较 π0.5 / GR00T N1.6 **~+40pp**; LIBERO **+0.1pp**；LIBERO-Plus 较 π0 **+30.6pp**；RoboCasa 较 π0 **+8.1pp**；SimplerEnv 较 π0 **+5.9pp**；
...
- **比 Pi0.5 好**：(25>17.8 SR@MolmoSpace, 可能`MolmoAct2-think`会更强)
...
- **SOTA**：**RoboTwin 2.0**：Clean **93.82%** / Random **93.30%**（50 双臂任务，自报 SOTA）,比GigaWorld-Policy(87%, 85%)好, 可惜STARRY没开源.
- **Benchmark**：—
...
- **SOTA**：**LIBERO SOTA 98.8%**（Specialist）；**RoboTwin 2.0 T2 88.3% / 87.7%**
...
- **SOTA**：**RoboTwin 2.0** (Clean:86%, Random:85%), 较 π0.5 **+95%**（如 Place Fan 0.25 → 0.94）
- **Benchmark**：—
...
- **简介**：Wan2.2-5B 视频 DiT backbone + 1B action expert；shared-attention MoT 结构化注意力 mask 解耦 video-cotraining 与 action 生成。**关键发现**：训练时视频 co-training 比测试时显式 future imagination 更重要。消融：单遍动作头 + 流匹配是 RoboTwin 高分关键。就是星海图(galaxea)出的.
...
- **数据规模**：LIBERO + RoboTwin 2.0 + 真机 Galaxea-R1-Lite 60h 毛巾折叠
...
以下网站汇总了各个VLA在不同的benchmark表现:
- https://allenai.github.io/vla-evaluation-harness/leaderboard/ , [vla-eval: A Unified Evaluation Harness for Vision-Language-Action Models](https://arxiv.org/abs/2603.13966), AllenAI的这个好像更新及时点
- https://tektonian.com/vla-benchmark 
...
| **LIBERO-Plus** | ACoT-VLA **87.5%** <br/> RLDX-1 **86.7%** | π₀.₅ (from ACoT-VLA)**86.5%** <br/> Being-H0.7 **84.8%** | OA-WAM **83.9%** | 全为 2026；π0 仅 56.1% |
| **LIBERO-Pro** | π0.5（LiberoPro论文测的）**62%** | GLaD **59.47%**（2025） | PRTS **58.8%** | \*评测论文条目，非模型本体 |
| **SimplerEnv** | Octo+PLD **96.63%**（2025） | FASTerVLA **87.9%** | — | X-VLA 报 Simpler-WidowX 95.8% / VM 80.4% / VA 75.7%（无单一聚合分）；2026 最高 GST-VLA 80.2% |
| **RoboTwin 2.0** | MotuBrain **95.8% / 96.1%** <br/> STARRY **93.82% / 93.30%** | LingBot-VA + Consistency-Consensus **93%** <br/> Fast-WAM **91.83%** | Being-H0.7 **90.2% / 89.6%** <br/> StarVLA-α **88.3% / 87.7** | Top3 全为 2026 / π 系 2026 |
| **RoboCasa** | X-WAM **79.2** <br/> World2Act(+GR00T N1.6) **72.6%** | RLDX-1 **70.6%** <br/> Cosmos Policy **67.1%** | HAMLET **66.4%**（300 demos）<br/> World2Act(+Cosmos Policy) **66.3%** | Top3 全为 2026 |
