# VLA 数据集 / 采集 / 处理全景分析(74 篇)

> 本文档是 [vla_traintask.md](vla_traintask.md)(训练任务范式)与 [vla_trainmdl.md](vla_trainmdl.md)(模型结构组件)的 **第三份姐妹篇**:从「数据集 + 数据采集方法 + 数据处理流水线 + 数据质量」视角再横切 74 篇 VLA / 具身论文(原 70 篇 + 2026-04 新增 X-WAM / MotuBrain、2026-05 新增 Consistency-Consensus、2026-01 新增 ACoT-VLA)。三份文件在第 7 章速查卡处 **三向互相链接**,形成「任务 × 模型 × 数据」三视角。
>
...
把 74 篇 VLA / 具身论文的"数据全景"摊在 7 大数据来源(D1-D7)的设计空间里,回答 4 个核心问题:
...
- **典型选择**:绝大多数 74 篇是「真机示教 + OXE 复用 + (可选)仿真 / 人类视频」组合。
...
**共同特点**:**站在社区肩上**,零采集成本启动;几乎全部 74 篇都至少做过 OXE / DROID / LIBERO 等的复用。
...
**核心结论(基于 74 篇规模数字与消融)**:三类数据**互补而非替代**;2026 主流是「**少量真机 + 大量仿真 + 海量人类视频**」的三段式配方。
...
- **1M+**:[MolmoB0T](#7d21-molmob0tpmolmob0t_large-scale_simulation_enables_zero-shot_manipulationpaperpdf--molmobot-engine-17m-专家轨迹) **1.7M sim** / [ABot-M0](#7d113-abot-m0pabot-m0_vla_foundation_model_with_action_manifold_learningpaperpdf--uniact-6m-跨-20-构型) **UniACT 6M** / [LingBot-VLA](#7d16-lingbot-vlaplingbot-vla__a_pragmatic_vla_foundation_modelpaperpdf--20000-小时真机双臂) **20K hours** / [X-WAM](#7d725-x-wampx-wam_unified_4d_world_action_modeling_from_video_priors_with_asynchronous_denoisingpaperpdf--5874-h-多源-robot-data--真机-20h-fine-tune) **5874 h / 1.49M episodes** / [Being-H0.5](#7d31-being-h05pbeing-h05paperpdf--unihand-35k-小时人手数据) **35K hours** / [Psi-R2](#7d33-psi-r2--psi-w0pfrom_human_skill_to_robotic_mastery_psi-r2__psi-w0pagehtml--10-万小时人类外骨骼--5417h-真机) **95,472 hours**。
- **Power-law 经验拟合**(基于 LingBot-VLA scaling 实测):\(\text{SR}(N) \approx 0.95 - 0.5 \cdot N^{-0.3}\),边际效用递减但**未饱和**(LingBot 3K → 20Kh 仍持续提升)。
- **多层金字塔范式**(2026 H1 新出现):[MotuBrain](#7d724-motubrainpmotubrain_an_advanced_world_action_model_for_robot_controlpaperpdf--四层数据金字塔--双臂异构--50-100-demo-适配) **4 层金字塔**(Internet 亿级 → ego-centric kh 级 → heterogeneous robot kh 级 → specific 50-100 demo)— **数据效率最高路径**:新本体 50-100 demo 即可适配,Stage1 视频 PT 与 Stage2 动作 PT 分别专注不同 modality;[X-WAM](#7d725-x-wampx-wam_unified_4d_world_action_modeling_from_video_priors_with_asynchronous_denoisingpaperpdf--5874-h-多源-robot-data--真机-20h-fine-tune) 验证 **RGB-D 4D 监督同时提升 3D 重建与策略 SR**(数据**模态**维度扩张比纯**规模**扩张更有效)。
- **零训练数据策略**:[Consistency-Consensus](#7d726-consistency-consensuspis_the_future_compatible_diagnosing_dynamic_consistency_in_world_action_modelspaperpdf--robocasa--robotwin-20-复用--零新数据) 验证 **复用现有 WAM checkpoint + 零新数据** 即可 **RoboTwin 2.0 +2.8 pp** — 暗示「数据已足、模型已能用、只缺更聪明的推理时选择」这一新维度。
...
- **Q1 自动 metric(SR / SPL / ACL)**:最广泛,但偏 benchmark;
  - 所有 74 篇都至少有一个;**LIBERO SR / RoboTwin SR / CALVIN ACL / SimplerEnv** 是事实标准;
...
**核心结论(基于 74 篇消融汇总)**:数据对各效果维度有**5 条黄金组合 + 5 条黑名单**。
...
flowchart TB
    ...
    psi --> motu["2026 H1 MotuBrain 4 层数据金字塔 (Internet→ego→hetero→specific 50-100 demo)"]
    psi --> xwam["2026 H1 X-WAM 5874 h robot + RGB-D 4D 监督"]
    fleet --> acot["2026 H1 ACoT-VLA LIBERO/LIBERO-Plus/VLABench 严格 split + 真机自采"]
    recap --> cc["2026 H1 Consistency-Consensus 复用 + 零新数据 +2.8 pp"]
...
**细化时间线**(74 篇里挑出每类数据代表 / 成熟工作,按近似发表时间):
...
- **2026 H1 — D4 + D3 + D1 多层金字塔范式**:[MotuBrain](#7d724-motubrainpmotubrain_an_advanced_world_action_model_for_robot_controlpaperpdf--四层数据金字塔--双臂异构--50-100-demo-适配) 4 层金字塔(Internet → ego-centric → heterogeneous → specific 50-100 demo);[X-WAM](#7d725-x-wampx-wam_unified_4d_world_action_modeling_from_video_priors_with_asynchronous_denoisingpaperpdf--5874-h-多源-robot-data--真机-20h-fine-tune) 5874 h robot data + RGB-D 4D 监督。
- **2026 H1 — D7 严格 split + 真机自采**:[ACoT-VLA](#7d727-acot-vlapacot-vla_action_chain-of-thought_for_vision-language-action_modelspaperpdf--liberolibero-plusvlabench--真机自采) 三 benchmark 严格走官方 split 验证 EAR + IAR 动作空间 CoT 价值。
- **2026 H1 — 零训练数据策略**:[Consistency-Consensus](#7d726-consistency-consensuspis_the_future_compatible_diagnosing_dynamic_consistency_in_world_action_modelspaperpdf--robocasa--robotwin-20-复用--零新数据) 复用现有 WAM checkpoint + 零新数据 → RoboTwin 2.0 +2.8 pp。
...
> - 7.D7 OXE/Benchmark 复用主(27 篇):FLOWER / SimVLA / VLANeXt / VLA-OPD / VLA-JEPA / MINT-4B / MolmoAct2 / X-VLA / Pose-VLA / GeneralVLA / PokéVLA / RLDX-1 / GST-VLA / PRTS / CycleVLA / STRONG-VLA / Green-VLA / NS-VLA / P3Nav / SACA / ReconVLA / OA-WAM / QuantVLA / **MotuBrain** / **X-WAM** / **Consistency-Consensus** / **ACoT-VLA**
...
### 7.D7 OXE / Benchmark 复用主 — 27 篇 [T2]
...
#### 7.D7.24 [MotuBrain](p/MotuBrain_An_Advanced_World_Action_Model_for_Robot_Control/paper.pdf) — 四层数据金字塔 + 双臂异构 + 50-100 demo 适配 [T1]

- **一句话定位**:**Vidu 基座 + 4 层数据金字塔(Internet → ego-centric → heterogeneous → specific)** 实现 RoboTwin 2.0 综合 #1
- **主数据来源**:**D4 视频基座(Vidu PT)** + **D3 ego-centric 人类视频** + **D1 异构双臂 robot data** + **D1 specific 50-100 demo**
- **数据规模**:`L1: Internet 亿级视频(Vidu 训练已完成);L2: ego-centric 视频(具体小时数原文未公开);L3: 异构双臂 robot data(只用双臂这一限定);L4: 新本体 50-100 demos 即可适配`(Sec 2.2)
- **采集方法**:**Vidu 基座已用 Internet 视频自训**;ego-centric 来自公开 + 自采;异构双臂用现成 OXE-like 整合 + 小米自家平台;specific 用 OEM 标准遥操作
- **处理流水线**:
  - 采集:四层独立采集(亿级 web → kh 级 ego → kh 级 robot → 100 demo specific)
  - 清洗:LingBot-VA noisy-conditioning(s_aug ∼ U[0.3, 0.7])+ 多视角随机 drop p=0.1
  - 标注:统一 Action 10-D(pos + 6D rot + gripper)相对 EEF 表示
  - Tokenize:Vidu VAE encode + condition image 作 first frame teacher-forced
  - 增广:多视角 3D RoPE 偏移(spatial only,temporal 不变)支持任意视角数
- **质量评估**:**RoboTwin 2.0 95.8% / 96.1%**(50 任务,clean / randomized);**Pick Dual Bottles 100% / Place A2B Left 95-100%**;**新本体 50-100 demo 即可工作**(Sec 4.4)
- **关键消融**:
  - 完整 4 层 PT → 95.8% / 96.1% 综合 #1
  - 仅 specific 50 demo(无 PT) → 大幅退化(具体数字详见 paper Table 5)
  - H-Bridge 中间联合层 → 跨模态对齐关键,decoupled 各层性能 -3 pp+
  - V2A 推理(仅 action 路径)与完整联合 → SR 仅低 sub-percent(Sec 2.4)
- **最重要数据决策 + 为什么**:**4 层金字塔 + 两阶段 PT**(Stage1 仅训视频分支、Stage2 训动作分支冻视频)— **数据效率**与**单一稳定 recipe** 双兼顾,新本体仅 50-100 demo 即可适配。
- **优势**:RoboTwin 2.0 几乎天花板;5 种推理模式同模型支持(VLA / WM / IDM / VGM / Joint)。**局限**:依赖闭源 Vidu 基座;具体每层数据精确小时数原文未完整公开。
- **三向链回**:task=[B4 World-Action 共演化 + A3 Flow + C4 Egocentric](vla_traintask.md) / mdl=[7.W.20 MotuBrain Vidu+三流 MoT+H-Bridge](vla_trainmdl.md) / 数据 **D4 + D3 + D1 四层金字塔**

#### 7.D7.25 [X-WAM](p/X-WAM_Unified_4D_World_Action_Modeling_from_Video_Priors_with_Asynchronous_Denoising/paper.pdf) — 5874 h 多源 robot data + 真机 20h fine-tune [T1]

- **一句话定位**:**Wan2.2-5B + 5874h robot data + 多视角 RGB-D** 实现 RoboCasa SOTA 79.2%
- **主数据来源**:**D1 真机 + D2 仿真** 联合 5874 h / 1.49M episodes;**D4 视频基座(Wan2.2-TI2V-5B)**
- **数据规模**:`5874 h / 1.49M episodes 真机 + 仿真`(Sec 3,paper §3.3);earphone packing 真机微调 **~20 h demo @ 320×256 分辨率**(Sec 4 real-world experiments)
- **采集方法**:多本体 robot data 整合(具体来源详见 paper §3 dataset);earphone packing 自采
- **处理流水线**:
  - 采集:1.49M episodes / 5874 h(全数据 unified preprocessing)
  - 清洗:**filtering** 删除 short / corrupted episodes;统一 320×256 分辨率
  - 标注:多视角 RGB + depth 联合(部分数据集需要 depth 估计或重建)
  - Tokenize:Wan VAE 编码 RGB latent + 深度 latent(共享 backbone,深度分支复制 final blocks)
  - 增广:**ANS 联合 timestep 采样**((t_O, t_a) joint dist,与推理分布对齐)
- **质量评估**:**RoboCasa 79.2%**(50 任务平均,Sec 4.2);**RoboTwin 2.0 90.7%**;**4D 重建 visual & geometric 指标均 SOTA**(超过 Cosmos-Policy / UWM / Motus)
- **关键消融**:
  - 加深度分支 → 同时提升 3D 重建质量 **+** 策略 SR(Sec 4.3,深度监督 multi-objective 受益)
  - ANS vs 独立采样 → 训练-推理分布对齐使加速无质量损失
  - 仅 Wan2.2-5B 微调(无 5874 h PT) → 直接 fine-tune 显著退化(ablation Sec 4.3)
- **最重要数据决策 + 为什么**:**5874 h 大规模 robot data PT + RGB-D 4D 监督** — 既扩 D 也扩 modality;深度作为 explicit 3D supervision 同时帮 3D 重建与策略学习,是「画质与具身有用性可同涨」的实证。
- **优势**:RoboCasa SOTA;首个 unified 4D WAM;真机 20h demo 即可适配新任务。**局限**:部分 robot data 无原生深度需估计;计算成本随深度分支线性增加。
- **三向链回**:task=[B4 + C4 + A3](vla_traintask.md) / mdl=[7.W.19 X-WAM Wan2.2 + 深度分支 + ANS](vla_trainmdl.md) / 数据 **D1 + D2 + D4 + RGB-D**

#### 7.D7.26 [Consistency-Consensus](p/Is_the_Future_Compatible_Diagnosing_Dynamic_Consistency_in_World_Action_Models/paper.pdf) — RoboCasa / RoboTwin 2.0 复用 + 零新数据 [T2]

- **一句话定位**:**无新数据 + 无训练**的 WAM test-time selection,**RoboTwin 2.0 +2.8 pp**
- **主数据来源**:**D7** 复用 RoboCasa + RoboTwin 2.0 已有 demos(Cosmos-Policy NVIDIA pretrain + LingBot-VA 小米 pretrain checkpoint)
- **数据规模**:**0 新训练数据**;评测覆盖 RoboCasa / RoboTwin 2.0 全任务(论文 §3.1 任务列表 ~30 双臂任务)
- **采集方法**:复用已有 benchmark
- **处理流水线**:
  - 采集:N/A(零训练)
  - 清洗:N/A
  - 标注:计算 action-state consistency \(c_t = \exp(-\alpha \cdot d_{\text{MSE-latent}})\)(α=0.1)
  - Tokenize:在 latent space 计算 MSE(VAE decode 前),避免 pixel-level 噪声
  - 增广:每步采 K 次 rollout 按 consistency 排序选最高
- **质量评估**:**Cohen's d = 0.76**(Cosmos-Policy joint pred)/ **0.99**(LingBot-VA inverse dyn);**AUC = 0.77 / 0.88**(成功-失败二分类);**RoboTwin 2.0 90.2 → 93.0 (+2.8 pp)**;**RoboCasa 66.6 → 67.3 (+0.7 pp)**
- **关键消融**:
  - 一致性 z-score 显著分离成功-失败 episode(Sec 3.1)
  - background collapse 失败模式:低 Δz_t 任务一致性反向(TurnOnSinkFaucet 等,Sec 3.2)
  - 与 learned value head 对照 → consistency 与 value 趋势同源(Sec 3.3)
- **最重要数据决策 + 为什么**:**复用现有 WAM checkpoint + 零训练**——验证「WAM 已具备内部一致性属性」,可被即插即用于 test-time ranking,**避免训 reward 模型的数据成本**。
- **优势**:无新数据、无新参数、无训练成本;可作 baseline 直接对照任何 WAM。**局限**:对 background collapse 类低动态任务需用 Δz_t 诊断后再用;K 次 rollout 增推理成本。
- **三向链回**:task=[D2 Value/Reward 替代 + B4 WAM 评估](vla_traintask.md) / mdl=[7.W.21 Consistency-Consensus 零训练 ranking](vla_trainmdl.md) / 数据 **复用 D7 RoboCasa+RoboTwin 2.0**

#### 7.D7.27 [ACoT-VLA](p/ACoT-VLA_Action_Chain-of-Thought_for_Vision-Language-Action_Models/paper.pdf) — LIBERO/LIBERO-Plus/VLABench + 真机自采 [T2]

- **一句话定位**:**LIBERO + LIBERO-Plus + VLABench 三 benchmark 严格官方 split + 真机自采**,LIBERO-Plus 87.5% 全局 #1
- **主数据来源**:**D7** LIBERO 四套件 + LIBERO-Plus 7 维度扰动套件 + VLABench 5 tracks + **D1 真机自采**
- **数据规模**:`LIBERO 四套件 50 demo/task × 40 task = 2000 episodes;LIBERO-Plus 10,030 episodes 7 维度扰动;VLABench 5 tracks 标准 split;真机自采自家平台(具体小时数详见 paper Appendix A)`(Sec 4.1)
- **采集方法**:严格走官方 split,无额外数据增广
- **处理流水线**:
  - 采集:LIBERO / LIBERO-Plus / VLABench 标准 + 真机自家平台
  - 清洗:N/A(走 benchmark 提供 split)
  - 标注:flow matching 训练目标(MSE)+ teacher-forcing stabilization(EAR 训练用 GT action,推理用 EAR 自身预测)
  - Tokenize:SigLIP 视觉 + Gemma-2B 文本 → π0.5 标准 pipeline
  - 增广:无(LIBERO-Plus 本身已是扰动套件)
- **质量评估**:**LIBERO 平均 +1.6 pp over π0.5**(LIBERO-Long 子集涨幅最大);**LIBERO-Plus 87.5% 全局 #1**;**VLABench IS 63.5% / PS 47.4%**(unseen-texture IS +12.6 pp / PS +7.2 pp)
- **关键消融**(Table 4-6):
  - EAR alone vs Baseline → +1.2 pp avg
  - IAR alone vs Baseline → +1.2 pp avg
  - EAR + IAR → +1.5 pp avg(两者互补)
  - Reference action config:H_ref=15 / shift=2 最佳(Table 5)
  - Teacher-forcing stabilization → 训练稳定性显著好
- **最重要数据决策 + 为什么**:**严格走 benchmark 官方 split + 真机自采补充** — 让方法贡献(EAR + IAR)完全归因于架构创新,不混淆数据增广效果;LIBERO-Long 提升尤其显著说明 action-space CoT 对长程任务更有用。
- **优势**:数据效率高(走标准 split 即得 SOTA);单卡 RTX 4090 推理。**局限**:仅 LIBERO / LIBERO-Plus / VLABench 仿真 + 自家真机平台,跨平台真机验证待补。
- **三向链回**:task=[A3 Flow + D5 CoT + F1 VLA 长程](vla_traintask.md) / mdl=[7.W.6 ACoT-VLA EAR+IAR](vla_trainmdl.md) / 数据 **D7 三 benchmark + D1 真机**
...
基于 74 篇 2024-2026 论文的数据级实证观察,下面 8 条趋势已在多篇工作中显现,并极可能延续到 2026~2027 年:
...
> 速查卡编号 `7.DX.N` 对应第 7 章 "74 篇数据卡" 中的同名子节;
...
- [ACoT-VLA](p/ACoT-VLA_Action_Chain-of-Thought_for_Vision-Language-Action_Models/paper.pdf) → 7.D7.27 · **D7+D1** · LIBERO / LIBERO-Plus / VLABench 严格 split + 真机自采
...
- [Consistency-Consensus (Is the Future Compatible?)](p/Is_the_Future_Compatible_Diagnosing_Dynamic_Consistency_in_World_Action_Models/paper.pdf) → 7.D7.26 · **D7** · RoboCasa + RoboTwin 2.0 复用 + 零新数据
...
- [MotuBrain](p/MotuBrain_An_Advanced_World_Action_Model_for_Robot_Control/paper.pdf) → 7.D7.24 · **D4+D3+D1** · 4 层数据金字塔(Internet → ego → hetero → specific 50-100 demo)
...
- [X-WAM](p/X-WAM_Unified_4D_World_Action_Modeling_from_Video_Priors_with_Asynchronous_Denoising/paper.pdf) → 7.D7.25 · **D1+D2+D4** · 5874 h / 1.49M episodes + RGB-D 4D 监督 + Wan2.2-5B
...
- ^{[ext4]} **LIBERO**: Liu et al., "LIBERO: Benchmarking Knowledge Transfer for Lifelong Robot Learning", arXiv 2306.03310 — 仿真长程操作 benchmark,74 篇中**>50 篇**使用。
...
> - 第 7 章:74 篇数据卡(按主数据来源分 7.D1-7.D7,共 ~70 子卡,每篇含一句话定位 / 主数据来源+规模数字 / 采集方法 / 处理流水线 / 质量评估 / 关键消融数字 / 最重要数据决策 + 为什么 / 优势-局限 / 三向链回 vla_traintask.md + vla_trainmdl.md)
> - 第 8 章:8 个场景化数据采集配方 + 10 条数据陷阱(均带 74 篇内证据 / 正面案例)+ 8 条未来趋势
> - 第 9 章:字母索引(74 篇)+ 三重倒排索引(主来源 D / 流水线 P / 创新点 I)+ 8 条数据域外部权威来源(去重前两份文档的 16 条)
>
